...
void vcpy_16_2x(void *dest, void *src, int x, int y, int pixels, int lines, int width) {
  int x2, y2; unsigned i;
  pcast(src, char*)+=y*width+x>>1;
  for (y2=0; y2<lines; y2++, pcast(src, char*)+=width>>1, pcast(dest, char*)+=width)
    for (x2=0; x2<pixels>>3; x2++) {
      i=pcast(src, unsigned long*)[x2];
      pcast(dest, long*)[x2*2]=(i&0xffff)+(i<<16);
      pcast(dest, long*)[x2*2+1]=(i>>16)+(i&0xffff0000);
    }
}
...
void v_copy() {
  ...
  if (vesastuf._h*vesastuf._w*2!=tmpsz) {
    if (tmpbf) hfree(tmpbf);
    tmpbf=halloc(tmpsz=vesastuf._h*vesastuf._w*2);
  }
/*  if (vesastuf._b==16 && !logoptr) {
  ...
  if (vesastuf._b!=16) cp=vcpy_16_08; else {
    cp=vcpy_n_n;
    if (dbl) cp=vcpy_16_2x;
  }
  bf=vesastuf._buf;
...
int test(int x, int y, int ysh, int b, int mnum) {
  ...
  if (vesastuf._buf) hfree(vesastuf._buf);
