...
int vesasetmode(int);
#pragma aux vesasetmode="mov eax,0x4f02" "int 0x10" parm [ebx] value [eax] modify [eax ebx ecx edx esi edi];
...
void calccube() {
  static char pal[1024];
  int i, c, e, e2, r, g, b, rr, gg, bb;
  _asm {
    xor ecx, ecx
    luup:
      mov edx, 3c7h
      ...
      add edx, 2
      in al, dx
      mov byte ptr[ecx*4+pal+0], al
      in al, dx
      mov byte ptr[ecx*4+pal+1], al
      in al, dx
      mov byte ptr[ecx*4+pal+2], al
      inc cl
    jnz luup
  };
  _asm {
  ...
  dosputs("Colormaps: $");
  for (i=0; i<65536; i++) {
    e=0x7fffffff; cube[i]=0;
    r=i>>10&0x3e; g=i>>5&0x3f; b=i<<1&0x3e;
    for (c=0; c<256; c++) {
      rr=r-pal[c*4+0];
      gg=g-pal[c*4+1];
      bb=b-pal[c*4+2];
      e2=rr*rr+gg*gg+bb*bb;
      if (e2<e) { cube[i]=c; e=e2; }
    }
    if ((i&0x3ff)==0x3ff) dosputs(".$");
  }
}
...
int test(int x, int y, int ysh, int b, int mnum) {
  if (mnum>=0x100) {
    vesastuf._vesa=1;
    if (vesasetmode(mnum)!=0x4f) return 0;
  } else {
    vesastuf._vesa=0;
    _asm {
      mov eax,13h
