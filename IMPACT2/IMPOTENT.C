...
char mappi2[65536], valospr[64*64];
...
void precalculoitcpaska() {
  ...
  for (y=0; y<64; y++) {
    ...
    for (x=0; x<256; x++) {
      r=rr*x>>9; g=gg*x>>9; b=bb*x>>9;
      paletti4[y*256+x]=(hmx(r)>>3<<11)+(hmx(g)>>2<<5)+(hmx(b)>>3);
    }
  }
  dosputs(".$");
...
void plussprite_c(int x1, int y1, int x2, int y2, int leveys, int korkeus,
  ...
  if (x1<0)       x1b=0;   else x1b=x1+255>>8;
  if (x2>160*256) x2b=160; else x2b=x2+255>>8;
  if (x1b>=x2b) return;
  if (y1<0)       y1b=0;   else y1b=y1+255>>8;
  if (y2>100*256) y2b=100; else y2b=y2+255>>8;
  if (y1b>=y2b) return;
  u16inc=16777216.0*leveys/(x2-x1);
  u16_2=((x1b<<8)-x1)*u16inc>>8;
  v16inc=16777216.0*korkeus/(y2-y1);
...
/*  for (x=0; x<64000; x++) {
  ...
  for (x=0; x<64000; x++) dest[x]=rtbl[(mappi2[(isox[x]<<8)+isoz[x]]*valo[x]>>6)+1024];
}
...
void teepistevektori(char *dest, int *datx, int *daty, int *datz, int dots,
  ...
  for (i=0; i<dots; i++) {
    ...
    z=24000/(uusiz[i]+0.5)*3000; //if (z>16384) z=16384;
    x1=x-z;
    y1=y-z;
    x2=x+z;
    y2=y+z;
...
/*  for (y=y1, v=0; y<y2; y++, v+=2) for (x=x1, u=0; x<x2; x++, u+=2) {
  ...
  for (y=y1, v=0; y<y2; y++, v+=2) {
    ...
    for (x=x1; x<x2; x++, d+=2, s++) {
      d[0]=s[0]+1>>1;
      d[1]=rtbl[(s[0]+s[1])*5-(s[-1]+s[2])+16392>>4];
    }
  }
...
//  if (kerroin<0) kerroin=0;
  if (musatimer()<15*64) s=z*0.25;
    else s=683.59375-(z*0.25-683.59375)*6.0;
  s*=0.75;
  a=aika*1024.0;
  for (y=0; y<80; y++) for (x=0; x<50; x++) if (sprite[y*50+x]>4) {
//    nx=(x-25)+fsin(aika+randtaul[y*50+x]/256)*kerroin;
//    ny=(y-40)+fsin(aika+randtaul[y*50+x+1]/256)*kerroin;
    nx=(x-25)+sintbl[a+randtaul[y*50+x]&4095]*kerroin;
    ny=(y-40)+sintbl[a+randtaul[y*50+x+1]&4095]*kerroin;
    i=nx*z*0.1+80*256;
    j=ny*z*0.1+50*256;
    plussprite_c(i-s, j-s, i+s, j+s, 64, 64, valospr, dest, sprite[y*50+x]<<2);
  }
}
...
///////////////////////////////////// flaret
      ...
      if (mdata->curord<8) {
        ...
        br=64; if (mdata->curord==7) br=63-mdata->currow;
...
///////////////////////////////////// taso+logot
      ...
      if (mdata->curord>7 && mdata->curord<12) {
        ...
        if (mdata->curord==9) for (x=16000; x<48000; x++) dbuf[x]=rtbl[(dbuf[x]+logo1[x])+1024];
        if (mdata->curord==10) for (x=16000; x<48000; x++) dbuf[x]=rtbl[(dbuf[x]+logo2[x])+1024];
...
///////////////////////////////////// flarekuva
      ...
      if (mdata->curord>11 && mdata->curord<16) {
        br=64;
        if (mdata->curord==12) br=mdata->currow;
        if (mdata->curord==15) br=63-mdata->currow;
        b=(musatimer()-12*64)/15.0;

        fill32(dbuf, 16000, 0);
        kuvazoomeri(dbuf, 35000/b, a/2.0, (15*64-musatimer())/100.0);
        suurenna(dbuf, dbuf2, 160, 100);
        teeharmaastavari(videomem, dbuf2, plasma1, paletti2, br*1024);
        v_copy();
...
///////////////////////////////////// flaret ja unzoomeri
      ...
      if (mdata->curord>19 && mdata->curord<24) {
        ...
        teeharmaastavari(videomem, dbuf2, plasma1, paletti4, br*1024);
        v_copy();
        unzoomaa(dbuf2, dbuf, 160, 100);
        teevaloja1(dbuf , fsin(a*0.444)*0.75, 0, 0,
                          -2.0+fsin(a*pi/8)*0.5, 0, 0, a/2, valom);
        teeharmaastavari(videomem, dbuf, plasma1, paletti4, br*1024);
        v_copy();
      }
