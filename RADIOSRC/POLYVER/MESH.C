...
Mesh *mesh_p_set4(Mesh *o, int i, Material *m, int v1, int v2, int v3, int v4) {
  Poly3d *p=o->pl[i];
  if (!p || p->vc<4) p=o->pl[i]=zgetmem(sizeof(Poly3d)+4*sizeof(Vertex3d*));
  p->m=m; p->vc=4;
  p->vl[0]=o->vl[v1]; p->vl[1]=o->vl[v2];
...
Mesh *mesh_p_setn(Mesh *o, int i, Material *m, int n) {
  Poly3d *p=o->pl[i];
  if (!p || p->vc<n) p=o->pl[i]=zgetmem(sizeof(Poly3d)+n*sizeof(Vertex3d*));
  p->m=m; p->vc=n;
  return o;
}
...
static void mesh_addedge(Mesh *o, Vertex3d *v1, Vertex3d *v2, Poly3d *p) {
  ...
  for (e=ecache[i]; e; e=e->next) if (e->v1==v1 && e->v2==v2) {
    if (e->p2) erreur("calcedge: edge shared with more than 2 polys");
    e->p2=p; break;
  }
  if (!e) {
    //e=o->el[o->ec++]=zgetmem(sizeof(Edge3d));
    o->ec++;
    e=zgetmem(sizeof(Edge3d));
...
Mesh *mesh_calc_edges(Mesh *o) {
  ...
  o->ael=zgetmem(sizeof(Edge3d*)*o->aec);
...
Mesh *mesh_calc_normals(Mesh *o) {
  ...
  if (j>o->vc || 1) o->vl=zgetmem(j*sizeof(Vertex3d*));
  o->vc=j;
  j=0;
...
Mesh *new_mesh_read(Phile *ph, Material *m) {
  ...
  for (i=0; i<o->pc; i++) {
    ...
    //if (vc!=3) erreur("new_mesh_read: only triangles accepted at the moment");
    mesh_p_setn(o, i, m, vc);
    for (j=0; j<vc; j++) {
      k+=phile_readdelta(ph);
      if (k>=o->vc) erreur("new_mesh_read: error in vertex data");
      o->pl[i]->vl[j]=o->vl[k];
    }
  }
