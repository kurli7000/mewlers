...
Texture  *_new_txt() { Texture *x=getmem(sizeof(*x)); return x; }
Texture  *_save_txt(Texture *x) { x=save(x, sizeof(*x));
  if (x->bits16l) x->bits16l=save(x->bits16l, 2<<2*x->size);
  if (x->mipnext) x->mipnext=save_txt(x->mipnext);
  return x;
}
Texture  *_destroy_txt(Texture *x) { return destroy(x); }


Texture *_new_txt_shrink(Texture *s) {
  ...
  *(d=new_txt())=*s;
  ...
  sz=1<<2*d->size;
  d->bits16l=getmem(sz+sz); dp=d->bits16l, sp=s->bits16l;
  ...
  #undef src
  #undef dst
  d->mipnext=_new_txt_shrink(d);
  return d;
}
int rand() { static int z=1234567; return (z=z*7654321+1)>>12&0x7fff; }
/*
Texture *_txt_load(Texture *t, void *data, int xs, int ys, int sb,
...
Texture *_new_txt_load_tga(char *fname, float gamma) {
  ...
  //pitch=3*hdr.width;
  size=pitch*hdr.height; tmp=malloc(size); if (!tmp) { erreur("memory!"); }
...
Texture *_new_txt_load_tga(char *fname, float gamma) {
  ...
  t=txt_load(new_txt(), tmp, hdr.width, hdr.height, hdr.bpp, 11, 16, gamma);
  free(tmp); return t;
}
...
Material *_new_mat() { Material *x=getmem(sizeof(*x)); x->refl=0; x->fill=filltri; return x; }
Material *_save_mat(Material *x) { x=save(x, sizeof(*x)); return x; }
Material *_destroy_mat(Material *x) { return destroy(x); }


static void mat_stuph(Material *m) { }
Material *_mat_set_txt1(Material *m, Texture *t) { m->txt1=*t; mat_stuph(m); return m; }
//Material *_mat_something(Material *m) { m->fill=filltri_cthru; return m; }
Material *_mat_set_space(Material *m, Space *s) { m->space=s; return m; }
Material *_mat_set_refl(Material *m) { m->refl=1; return m; }
Material *_mat_set_filler(Material *m, void *filler) { m->fill=filler; return m; }
...
void glotx(char *_v, char *_a, char *_b) {
  int a=getbits(2), b, c;
  float f, ag, bg;
  if (a==0) a=11, b=-3;
    else if (a==1) a=5, b=3;
