...
Mesh *mesh_v_set(Mesh *o, int i, float x, float y, float z, float u, float v) {
  Vertex3d *w=o->vl[i]=newobjz(Vertex3d);
...
Mesh *mesh_check(Mesh *o) {
  Vertex3d *v, **v_, *v2, **v2_;
  Poly3d *p, **p_;
  int i, j;
  //err("tah");
  vertfor(v, o->vl, o->vc) if (!v) err("no vert");
  polyfor(p, o->pl, o->pc) if (!p) err("no poly");
  polyfor(p, o->pl, o->pc) vertfor(v, p->vl, p->vc) {
    vertfor(v2, p->vl, p->vc) if (v_!=v2_ && v==v2) err("dup verts");
    if (!v) err("no vert in poly");
    if (isnan(v->pos.x)||isnan(v->pos.y)||isnan(v->pos.z)) err("pos");
  }
  vertfor(v, o->vl, o->vc) v->flag=0;
  polyfor(p, o->pl, o->pc) vertfor(v, p->vl, p->vc) v->flag=1;
  vertfor(v, o->vl, o->vc) if (!v->flag) err("vert in vlist unused in polydata");
  polyfor(p, o->pl, o->pc) vertfor(v, p->vl, p->vc) v->flag=0;
  vertfor(v, o->vl, o->vc) v->flag=1;
  polyfor(p, o->pl, o->pc) vertfor(v, p->vl, p->vc)
    if (!v->flag) err("vert in polydata not added to vlist");
  return o;
}
Mesh *mesh_recalc(Mesh *o) {
  return /*mesh_calc_edges*/mesh_check(mesh_calc_normals(mesh_calc_bound(mesh_killempty(o))));
}
