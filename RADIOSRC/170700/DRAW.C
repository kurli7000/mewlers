...
void drawl(Poly3d **pp, int pc) {
  ...
  for (pm=pp+pc; pp<pm; pp++) {
    ...
    y0>>=8; y1>>=8;
    if (y0<0) y0=0; if (y1>1024) y1=1024;
    s0=scanp-y0;
    v0=p->vl[0]->v2d;
    v1=p->vl[1]->v2d;
    v2=p->vl[2]->v2d;
    ala=256.0/((v1->x-v0->x)*1.*(v2->y-v0->y)-(v2->x-v0->x)*1.*(v1->y-v0->y));
    ua=round(((v1->u-v0->u)*1.*(v2->y-v0->y)-(v2->u-v0->u)*1.*(v1->y-v0->y))*ala);
    ub=round(((v2->u-v0->u)*1.*(v1->x-v0->x)-(v1->u-v0->u)*1.*(v2->x-v0->x))*ala);
    va=round(((v1->v-v0->v)*1.*(v2->y-v0->y)-(v2->v-v0->v)*1.*(v1->y-v0->y))*ala);
    vb=round(((v2->v-v0->v)*1.*(v1->x-v0->x)-(v1->v-v0->v)*1.*(v2->x-v0->x))*ala);
    for (j=p->vc-1, i=0; i<p->vc; j=i++) {
      v1=p->vl[j]->v2d; v2=p->vl[i]->v2d;
      if (v1->y<v2->y) {
        kk=round((v2->x-v1->x)*4096./(v2->y-v1->y));
        x=v1->x*16+(kk*(256-(v1->y&255))>>8);
        for (y=v1->y>>8; y<v2->y>>8; y++) /*s0[y].x0d=kk, */s0[y].x0=x, x+=kk;
      }
    }
    for (j=p->vc-1, i=0; i<p->vc; j=i++) {
      v1=p->vl[j]->v2d; v2=p->vl[i]->v2d;
      if (v1->y>v2->y) {
        ...
        for (y=v2->y>>8; y<v1->y>>8; y++) {
          if (x>s0[y].x0) s0[y].x1=x/*, s0[y].x1d=kk*/;
            else s0[y].x1=s0[y].x0, /*s0[y].x1d=s0[y].x0d, */s0[y].x0=x/*, s0[y].x0d=kk*/;
          x+=kk;
        }
      }
    }
    u=v0->u+round(ub*(y0-v0->y*(1./256)+1)-ua*v0->x*(1./256));
    v=v0->v+round(vb*(y0-v0->y*(1./256)+1)-va*v0->x*(1./256));
    for (y=y0; y<y1; y++) {
      s0[y].z0=v0->z+v2->z;
      s0[y].txt=p->m->txt1.bits16l;
      s0[y].y=y;
...
void mesh_draw(Space *f, Mesh *o2, Matrix *m, unsigned flgs) {
  ...
  cur_f=*f;
  val++;
  o=mesh_clip_in(o2, clip_pl, clip_plm-clip_pl);
  if (!o) return;
  o=mesh_backface(o, &mf_inv.l);
  if (!o) return;
  xforml(o->vl, o->vc);
